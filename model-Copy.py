...
class CycleGAN(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm='inorm'):
        ...
        self.enc1 = CBR2d(in_channels, 1 * nker, kernel_size=7, padding=(7-1)//2,
                          norm=norm, relu=0.0, stride=1)
        
        self.enc2 = CBR2d(1* nker, 2 * nker, kernel_size=3, padding=(3-1)//2,
                          norm=norm, relu=0.0, stride=2)
        self.enc3 = CBR2d(2 * nker, 4 * nker, kernel_size=3, padding=(3-1)//2,
                          norm=norm, relu=0.0, stride=2)
        
        res = []
        for i in range(9):
            res += [ResBlock(4 * nker, 4 * nker, kernel_size=3, padding=(3-1)//2,
                             norm=norm, relu=0.0, stride=1)]
        self.res = nn.Sequential(*res)
        self.dec1 = DECBR2d(4 * nker, 2 * nker, kernel_size=3, padding=(3-1)//2,
                            norm=norm, relu=0.0, stride=2)
        self.dec2 = DECBR2d(2 * nker, 1 * nker, kernel_size=3, padding=(3-1)//2,
                            norm=norm, relu=0.0, stride=2)
        self.dec3 = CBR2d(1 * nker, out_channels, kernel_size=7, padding=(7-1)//2,
                            norm=None, relu=None, stride=1)
